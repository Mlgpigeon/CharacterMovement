...
public class ThirdPersonPlayer : MonoBehaviour
{
    ...
    private void Start()
    {
        ...
        _input = GetComponent<InputChecker>();
    ...
    private void Update()
    {
        //_input.SprintInput(_keyboard.leftShiftKey.IsPressed());
        CheckHurt();
        JumpAndGravity();
        GroundedCheck();
        CameraRotation();
    ...
    {
        if (_input.hurt && _animator.GetLayerWeight(1) < 1.0f)
        {
            _animator.SetLayerWeight(1,1.0f);
        }
        else if(!_input.hurt && _animator.GetLayerWeight(1) > 0.0f)
        {
    ...
    private void Move()
    {
        ...
        if (!_input.aim){
            if (_input.move != Vector2.zero)
            {
                ...
                if (!_animator.GetBool("inIdle") && !_animator.GetBool("superIdle"))
                {
                    //print("Done");
                }
    ...
    private void JumpAndGravity()
        {
            if (grounded)
            {
                ...
                //print(_animator.GetLayerWeight(1));
                // Jump
